...
async function handleGeneration(request, sendResponse) {
  try {
    ...
    if (type === "rewrite") {
      ...
      prompt = withStyle(`You are writing a clever, light roast in response to a public post.\n\nRead the post below and write a concise reply that:\n- Uses wit or irony to challenge the idea, not the person\n- Is playful, intelligent, and sharp — not rude or abusive\n- Avoids personal attacks, insults, or offensive language\n- Does NOT target appearance, identity, or character\n- Does NOT summarize or restate the post\n- Is concise (1–2 sentences max)\n- Sounds confident and humorous, like a smart human comment\n- Avoids emojis and hashtags\n\nThe roast should feel clever enough to spark reactions, not hostility.\n\nReturn only the reply text.\n\nPost:\n${text}`);
    } else if (type === "reject") {
